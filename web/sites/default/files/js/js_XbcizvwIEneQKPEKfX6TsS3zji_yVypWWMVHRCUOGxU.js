...
(($,Drupal)=>{Drupal.behaviors.graphqlComposeFormSummaries={attach(context){$('.entity-type-tab',context).drupalSetSummary(Drupal.behaviors.graphqlComposeFormSummaries.getMessage);$('.entity-bundle-tab',context).drupalSetSummary(Drupal.behaviors.graphqlComposeFormSummaries.getMessage);}};Drupal.behaviors.graphqlComposeFormSummaries.getMessage=(element)=>{const enabled=element.querySelectorAll('.entity-bundle-enabled:checked');const errors=element.querySelectorAll('.has-error');if(errors.length){const span=document.createElement('span');span.classList.add('has-nested-errors');span.innerText=Drupal.formatPlural(errors.length,'1 error','@count errors');return span.outerHTML;}return enabled.length?Drupal.t('Enabled'):'';};})(jQuery,Drupal);;
